...
\begin{itemize}
\item L'utilisateur écrit sa chorégraphie par une suite d'action séparé par un retour à la ligne.
...
\end{block}
\begin{itemize}
\item 1er paramétre est la durée du mouvement, en Seconde.
\item 2éme paramétre est la vitesse du mouvement, en pourcentage. Il représente la vitesse du drone par rapport à la vitesse définie dans la section "prologue".
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Notre langage intégre un mécanisme d'exécution d'instructions paralléle.
...
\item Le contenu de ce bloc d'instruction sera exécuter.
...
\end{column}
\begin{column}{0.5\textwidth}
Les bibliothéques de fonctions (instruction import):\\
\begin{itemize}
