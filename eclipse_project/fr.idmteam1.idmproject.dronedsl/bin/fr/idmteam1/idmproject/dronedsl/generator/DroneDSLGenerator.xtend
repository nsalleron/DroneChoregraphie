...
class DroneDSLGenerator extends AbstractGenerator implements IOutputConfigurationProvider {
	...
	val mainStubFilesMap = newLinkedHashMap(
	...
	val libStubFilesMap = newLinkedHashMap(
		"../src/fr/roboticiens/body/BodyInstruction.java" -> contentBodyInstructionClass(),
		"../src/fr/roboticiens/commandes/Atterrir.java" -> contentAtterrirClass(),
		"../src/fr/roboticiens/commandes/Avancer.java" -> contentAvancerClass(),
		"../src/fr/roboticiens/commandes/CommandeAvecDureeVitesse.java" -> contentCommandeAvecDureeVitesseClass(),
	...
	override doGenerate(Resource input, IFileSystemAccess2 fsa, IGeneratorContext context) {
		...
		if(!input.allContents.toIterable.filter(fr.idmteam1.idmproject.dronedsl.droneDSL.Model).empty) {
			generateMainStubFiles(fsa)
			for (m : input.allContents.toIterable.filter(fr.idmteam1.idmproject.dronedsl.droneDSL.Model)) {
				fsa.generateFile(mainFilePath, m.compile)
			}
		} else if(!input.allContents.toIterable.filter(fr.idmteam1.idmproject.dronedsl.droneDSLLib.Model).empty){
			generateLibStubFiles(fsa)
			
			for (m : input.allContents.toIterable.filter(fr.idmteam1.idmproject.dronedsl.droneDSLLib.Model)) {
	...
	def generateMainStubFiles(IFileSystemAccess2 fsa) {
		var itr = mainStubFilesMap.entrySet().iterator();
		while(itr.hasNext()) {
			var entry = itr.next()
	...
	def generateLibStubFiles(IFileSystemAccess2 fsa) {
		var itr = libStubFilesMap.entrySet().iterator();
		while(itr.hasNext()) {
			var entry = itr.next()
			var filepath = entry.key
			var content = entry.value
	...
	def compile(fr.idmteam1.idmproject.dronedsl.droneDSL.Model e) 
		''' 
		...
		public class Main {
			static DroneRuntime runtime = new ParrotDroneRuntime(System.getenv("ROBOTICIENS_PATH")+"/les_roboticiens_runtime_bebop2");
			«IF e.prologue!== null»
			                     « e.prologue.toString»
			«ENDIF»
    ...
    def compile(fr.idmteam1.idmproject.dronedsl.droneDSLLib.Model e) 
		''' 
		package fr.roboticiens;
    ...
    def compile(fr.idmteam1.idmproject.dronedsl.droneDSL.FonctionDecl e)
	...
	def compile(fr.idmteam1.idmproject.dronedsl.droneDSLLib.FonctionDecl e)
    	'''	public static void «e.name»(DroneRuntime runtime) {
		« FOR f:e.body»
	«f.toString»
		«ENDFOR»
